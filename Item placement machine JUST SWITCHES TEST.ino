//https://wokwi.com/projects/348073263045280338
...
volatile unsigned long previousStartStateChangeMillis = 0;
volatile unsigned long previousHomeStateChangeMillis = 0;
...
String notice="";
    ...
    (stock_in_place ? notice = "y |" : notice = "n |"   );
    LCD.setCursor(0, 1);
    LCD.print( notice);
    
    (product_shelf_home ? notice = "y" :  notice = "n");
      LCD.setCursor(4, 1);
      LCD.print( notice);
 
    (product_shelf_limit ? notice = "y |" :  notice = "n |");
      LCD.setCursor(8, 1);
      LCD.print( notice);

    (table_bar_home ? notice = "y" :  notice = "n ");
         LCD.setCursor(11, 1);
      LCD.print( notice);
  
    (table_bar_limit ? notice = "y |" :  notice = "n |");
         LCD.setCursor(15, 1);
      LCD.print( notice);

    (emergency_stop ? notice = "y" :  notice = "n ");
         LCD.setCursor(19, 1);
      LCD.print( notice);
...
bool HOMEpinState = digitalRead(OPERATOR_HOME_SWITCH_PIN);
  ...
  if (previous_home_pushed != HOMEpinState)
  {
    if (HOMEpinState  == HIGH)
    {
      if ((millis() - previousHomeStateChangeMillis) > debounceTime)
      {
        home_pushed = true;

      }
    }
    previousHomeStateChangeMillis = millis();
    previous_home_pushed = HOMEpinState;
  }


  bool STARTpinState = digitalRead(OPERATOR_START_SWITCH_PIN);
  
  if (previous_start_pushed != STARTpinState)
  {
    if (STARTpinState  == HIGH)
    {
      if ((millis() - previousStartStateChangeMillis) > debounceTime)
      {
        start_pushed = true;

      }
    }
    previousStartStateChangeMillis = millis();
    previous_start_pushed = STARTpinState;
  }
...
void EMERGENCY_STOP_Interrupt()
{
  ...
  TABLE_BAR.stop();
  PRODUCT_SHELF.stop();
  detachInterrupt(digitalPinToInterrupt(OPERATOR_EMERGENCY_STOP_INPUT_PIN));
  while (1);
}
